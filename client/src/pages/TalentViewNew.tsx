...
export default function TalentViewNew() {
  ...
  console.log('TalentViewNew (Studio A) rendering, callLines:', callLines);
  ...
  useEffect(() => {
    if (websocket && websocket.readyState === WebSocket.OPEN) {
      ...
      websocket.send(JSON.stringify({
        type: 'auth',
        role: 'talent',
        studioId: 'A' // Explicitly set Studio A for this talent view
      }));
      
      console.log('TalentViewA: Sent authentication with studioId: A');
      
      // Request chat messages
      websocket.send(JSON.stringify({
        type: 'getChatMessages',
        data: { role: 'talent', studioId: 'A' }
      }));
  ...
  useEffect(() => {
    ...
    const handleWebSocketMessage = (event: MessageEvent) => {
      ...
      console.log(`TalentViewA WebSocket message: ${data.type}`, data);
      
      // Process countdown messages (handled by CountdownContext automatically)
      // This comment is just for documentation as the actual handling is in the context
      // We don't need explicit handling here because useCountdown is already set up correctly
      
      // Handle producer buzzer to talent notification
      if (data.type === 'talentBuzzer') {
        console.log('⚠️ TALENT received talentBuzzer message:', JSON.stringify(data));
        const buzzerStudioId = data.data?.studioId;
        const activate = data.data?.activate !== undefined ? data.data.activate : false;
        
        // Only handle messages for Studio A in this component
        if (buzzerStudioId === 'A') {
          console.log(`⚠️ TALENT dispatching talentBuzzerEvent for Studio A: ${activate}`);
      ...
      if (data.type === 'chatHistory' && data.studioId === 'A') {
        console.log('Received chat history for Studio A:', data.messages);
      ...
      if (data.type === 'newChatMessage' && data.data && data.data.studioId === 'A') {
        console.log('Received new chat message for Studio A:', data.data);
  ...
  const clearChat = () => {
    ...
    console.log('Talent A: Clearing chat messages permanently');
      ...
      <div className="flex flex-col min-h-screen bg-zinc-900 text-zinc-100">
        ...
        <div className="bg-black text-white py-4 border-t border-b border-zinc-800 shadow-lg">
          <div className="relative">
            {/* Studio label at top */}
            <div className="absolute top-0 left-4 text-sm text-blue-400 font-medium">
              STUDIO A
            </div>
            ...
            <div className="flex justify-center">
              <div className="flex flex-col items-center">
                <div className="px-6 py-3 rounded-md mb-1 scale-110">
                  {/* The StandaloneTimer manages its own state */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 space-y-5">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-white">Studio A Call Lines</h2>
                <Badge variant="outline" className="bg-blue-900/60 text-white font-medium border-blue-700">
                  Active Call Console
                </Badge>
              </div>
              <div className="space-y-5 bg-zinc-800 p-5 rounded-xl shadow-md border border-zinc-700">
                ...
                {callLines
                  .filter(line => line.id <= 3) // Only show Studio A lines (1-3)
                  .some(line => line.status !== 'inactive') ? (
                    // If at least one line is active, show only active lines
                    callLines
                      .filter(line => line.id <= 3 && line.status !== 'inactive')
                      .map(line => (
                  ...
                  <div className="flex flex-col h-[600px]">
                    ...
                    <div className="bg-gradient-to-r from-zinc-900 to-zinc-800 p-4 border-b border-zinc-700">
                      <div className="flex items-center">
                        <Avatar className="mr-3 h-10 w-10 bg-zinc-700 border-2 border-zinc-600 shadow-md">
                          <AvatarFallback className="font-bold text-zinc-200">P</AvatarFallback>
                        </Avatar>
                        <div>
                    ...
                    <ScrollArea className="flex-1 p-5 bg-gradient-to-b from-zinc-900 to-zinc-800">
                      <div className="space-y-4">
                        {chatMessages.map(msg => (
                          ...
                          >
                            {msg.senderRole !== role && (
                              <Avatar className="h-8 w-8 bg-blue-900 border-2 border-zinc-700 shadow-md">
                                <AvatarFallback className="text-xs font-bold text-blue-100">P</AvatarFallback>
                              </Avatar>
                            )}
                            ...
                            >
                              ...
                              {msg.fileAttachment && (
                                <div className={`mt-2 p-2 rounded flex items-center ${
                                  msg.senderRole === role ? 'bg-blue-800' : 'bg-zinc-800'
                                }`}>
                                  ...
                                  <a 
                                    ...
                                    className={`ml-2 text-xs underline ${
                                      msg.senderRole === role ? 'text-blue-200' : 'text-blue-300'
                                    }`}
                                  >
                                    View
                                  </a>
                              ...
                              <p className={`text-xs mt-1 ${
                                msg.senderRole === role ? 'text-blue-200' : 'text-zinc-400'
                              }`}>
                            ...
                            {msg.senderRole === role && (
                              <Avatar className="h-8 w-8 bg-blue-600 border-2 border-zinc-700 shadow-md">
                                <AvatarFallback className="text-xs font-bold text-blue-100">TA</AvatarFallback>
                              </Avatar>
                            )}
                          </div>
                        ))}
                    ...
                    <div className="p-4 border-t border-zinc-700 bg-zinc-800">
                      {selectedFile && (
                        <div className="mb-3 p-2 bg-blue-900/40 rounded-md flex items-center justify-between border border-blue-800">
                          <div className="flex items-center">
                            <FileText className="h-4 w-4 mr-2 text-blue-400" />
                            <span className="text-sm truncate max-w-[200px] text-blue-300">
                              {selectedFile.name}
                            </span>
                          </div>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 rounded-full hover:bg-blue-800/50 text-blue-300"
                      ...
                      <div className="flex space-x-2">
                        <Input 
                          ...
                          className="bg-zinc-700 border-zinc-600 text-zinc-100 focus:border-blue-600 placeholder:text-zinc-400"
                        />
                        <input
                          type="file"
                        ...
                        <Button 
                          variant="outline" 
                          size="icon"
                          className="bg-zinc-700 border-zinc-600 text-zinc-300 hover:bg-blue-800 hover:text-blue-200"
  ...
  return (
    <Card className={`overflow-hidden shadow-md transition-all duration-200 bg-zinc-800 ${callLine.status !== 'inactive' ? `border ${statusStyle.border}` : 'border-zinc-700'}`}>
      <CardContent className="p-0">
        ...
        <div className={`p-3 border-b ${callLine.status !== 'inactive' ? 'border-blue-700/40' : 'border-zinc-700'}`}>
          <div className="flex items-center justify-between">
